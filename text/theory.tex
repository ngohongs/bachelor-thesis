        ...
        \end{itemize}
            ...
            \subsection{Částicové metody}  
                ...
                \subsubsection{Simulace podle Smoothed Particle Hydrodynamics}
                    ...
                    Obvyklá volba jádra $W$, které lze využít až na dvě vyjímky téměř pro každou interpolaci veličin v simulaci tekutin, je kernel \emph{poly6} \cite{Bridson2007, Mueller2003}
                    ...
                    \noindent \emph{Poly6} ale nekoretně aproximuje síly tlakového pole, v simulaci se částice pod vysokým tlakem nefyzikálně shlukují \cite{Mueller2008}, neboť $\nabla W$, jež je nutné pro výpočet síly způsobené rozdílem tlaku (rovnice \ref{eq:momentum_conservation}), se ve středu jádra blíží k nule, což vede k zanadbání odpudivých sil. Desburn řešil tento problém nahrazením jádra \emph{poly6} za kernel \cite{Desburn1996}
                    ...
                    Pohyb částíce je následně určený silami vyvolané rozdílem tlaků $-\nabla p$, viskozity tekutiny $\mu \nabla^2 \vec{u}$ a gravitací $\rho \vec{g}$ (na vodu působí i extérní síly, ty lze zakomponovat s gravitací do $\vec{g}$.
                    
                    Částice si ale udržují informace jen o své pozici, rychlosti a hmontnosti, zbylé vlastnosti si musí simulátor spočítat sám. Každá z nich určuje nějaký objem $V_i = \frac{m_i}{\rho_i} $, zatímco hmotnost zůstává během celé simulace konstantní, $\rho_i$ se musí evaluovat v každé vykreslovací smíčce \cite{Mueller2003}. Hustota $\rho_i$ částice $i$ na pozici $\mathbf{x}_i$ je pak podle SPH \ref{eq:sph} určená jako
                    
                    \begin{equation}
                        \rho(\mathbf{x}) = \sum_{j} m_j \frac{\rho_j}{\rho_j} W(| \mathbf{x} - \mathbf{x}_j|, h) = \sum_{j} m_j W(| \mathbf{x} - \mathbf{x}_j|, h).
                    \end{equation}
                   ...
                   S výše uvedenými informacemi lze nakonec spočítat síly působící v tekutině $- \nabla p$ a $\mu \nabla^2 \vec{u}$. Na základě SPH \ref{eq:sph} je tlaková síla působící na částici $i$ rovna
                   
                   \begin{equation}
                        \mathbf{f}_{i}^{tlak} = - \nabla p(\mathbf{x}_i) = - \sum_j m_j \frac{p_j}{\rho_j} \nabla W(|\mathbf{x}_i - \mathbf{x}_j|, h).
                   \end{equation}

                    \noindent Bohužel takto vypočítané síly nejsou symetrické (nesplňují Newtonův třetí zákon) \cite{Mueller2003}, tento problém je nejvíce patrný, když simulace obsahuje jen dvě částice. Protože gradient smoothing kernelu je v středu, tj. na pozici  částice $i$, roven nule, částice použije pro výpočet jen tlak částice $j$. Müller řeší tento problém jednoduchým průměrovaním hodnot $p_i$ a $p_j$
                   ...
                   \begin{equation}
                        \mathbf{f}_{i}^{viskozita} = \mu \nabla^2 \vec{u}(\mathbf{x}_i) = \mu \sum_j m_j \frac{u_j}{\rho_j} \nabla^2 W(|\mathbf{x}_i - \mathbf{x}_j|, h),
                   \end{equation}
                    ...
                    \begin{equation}
                        \mathbf{f}_{i}^{viskozita} = \mu \nabla^2 \vec{u}(\mathbf{x}_i) = \mu \sum_j m_j \frac{u_j - u_i}{\rho_j} \nabla^2 W(|\mathbf{x}_i - \mathbf{x}_j|, h),
                    \end{equation}
                    
                    Gravitace a externí síly jsou přímo aplikovány na samotné částice. Se všemi těmito informacemi lze vypočítat NSE, které lze formulovat jako Newtonovým druhým zákonem
                    
                    \begin{equation}
                        \mathbf{a}_i = \frac{ D \mathbf{u} }{ Dt } = \frac{\mathbf{f}_i}{ \rho_i },
                    \end{equation}
                    
                    kde $a_i$ je zrychlení, $\rho_i$ je hustota a $\mathbf{f}_i$ součet sil působící na částici $i$. Pro získání rychlosti nyní stačí integrovat podle času zrychlení $\mathbf{a}_i$. Müllerův simulátor \cite{Mueller2003} pro integraci používal algoritmus Leap--Frog \cite{Pozrikidis2008}, který integruje podle konstantní časových rozestupů, pro lepší výsledky nabídl algoritmy založené na Courant--Friedrichs--Lewy podmínce \cite{Desburn1996}.
                    
                    Posledním krokem je vizualizace částic. Bez žádné speciální metody by částice měly pouhý tvar kuliček, které nejsou nijak spojeny. Řešením je tzv. point splatting \cite{Zwicker2001}, který na základě množiny bodů vytvoří jednolitý útvar.
