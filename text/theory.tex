        ...
        \end{itemize}
            ...
            \subsection{Částicové metody}  
                ...
                \subsubsection{Simulace podle Smoothed Particle Hydrodynamics}
                    ...
                    Částice si ale udržují informace jen o své pozici, rychlosti a hustotě, zbylé vlastnosti si musí simulátor spočítat sám. Tlak $p$ v částici lze aproximovat pomocí stavové rovnice ideálního plynu \cite{Mueller2003} 
                    
                    \begin{equation}
                        p = k \rho, \label{eq:ideal_state}
                    \end{equation}
                    
                   \noindent kde $k$ je konstanta plynu závislá na teplotě a $\rho$ hustota částice. Desburn navrhl úpravu rovnice \ref{eq:ideal_state}, která přispěla k větší stabilitě simulace \cite{Desburn1996}
                   
                    \begin{equation}
                        p = k (\rho - \rho_0), 
                    \end{equation}
                   
                   \noindent kde $\rho_0$ označuje klidovou hustotu tekutiny. Protože síla vyvolaná rozdílem tlaku závisí na gradientu tlaku je toto řešení validní.
                   
                    Pak podle SPH \ref{eq:sph} je pole hustot (nutné pro kalkulaci tlaku) na pozici $\mathbf{x}$ určené jako
