            ...
            \subsection{Částicové metody}  
                ...
                \subsection{Hybridní metody}
                    ...
                    \subsubsection{Simulace podle vlnové rovnice}
                        ...
                        Nechť funkce $u(x,t)$ udává, jak je struna na pozici $x$ v čase $t$ vychýlená od osy x . V případě, že je struna v klidové stavu, struna splývá s osou x. Dále nechť na segment struny $AB$, část sturny od $x$ do $x + \Delta x$, kde $\Delta x \xrightarrow{} 0 $, působí napínací síly $\mathbf{F}_{T_1}$ a $\mathbf{F}_{T_2}$. Za překlodu, že se segement $AB$ málo vychyluje od osy x, lze jejich velikosti sil považovat za identické, 
                        $\mathbf{F}_{T_1} = \mathbf{F}_{T_2} = \mathbf{F}_T$ \cite{Lewin2008}. Kvůli čtvrtému předpokladu \ref{en:wave_as} se horizontální síly vyruší a stačí spočítat síly působící ve vertikální ose.
                        
                        \begin{equation}
                            \mathbf{F}_{vert} = -\mathbf{F}_T \sin(\theta) + \mathbf{F}_T \sin(\theta + \Delta \theta)
                        \end{equation}
                        
                        \noindent Síla $-\mathbf{F}_T \sin(\theta)$ působí u bodu $A$ směrem dolů a síla $\mathbf{F}_T \sin(\theta + \Delta \theta)$ u bodu $B$ opačným směrem. Díky pátemu předpokladu \ref{en:wave_as} lze aproximovat $\sin(\theta)$ a $\sin(\theta + \Delta \theta)$ jako velikost jejich úhlu.
                        
                        \begin{equation}
                            \mathbf{F}_{vert} = -\mathbf{F}_T \theta + \mathbf{F}_T (\theta + \Delta \theta) = \mathbf{F}_T \Delta \theta
                        \end{equation}
                        
                        \noindent Po získání síly působící na segment $AB$ lze aplikovat Newtonův druhý zákon.
                        
                        \begin{equation}
                            \mathbf{F}_{vert} = m \mathbf{a} \\
                        \end{equation}
                        \begin{equation}
                            \mathbf{F}_{T} \Delta \theta = (\Delta x \mu) \frac{\partial^2 u}{\partial t^2} \\
                        \end{equation}
                        
                        \noindent $\mu$ značí hmotnost na jednotku délky. Zrychlení $\mathbf{a}$ lze přepsat jako druhou derivací funkce $u$ podle $t$. Dále pro $ \Delta x \xrightarrow{} 0$ platí, že
                        
                        \begin{equation}
                            \tg(\theta) = \frac{\partial u}{\partial x}
                        \end{equation}
                        
                        \noindent a pro derivaci $\tg(\theta)$ podle x
                        
                        \begin{equation}
                            \frac{1}{\cos^2(\theta)} \frac{d \theta}{dx} = \frac{\partial u}{\partial x}
                        \end{equation}
