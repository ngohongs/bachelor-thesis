...
Během celé simulace opakuj
   ...
   Pro každé i, j proveď
    ...
    \section{Optické vlastnosti}
        ...
        U každého objektu ve světě způsob, jak je naše oko vnímá, zcela závisí na světle, jak intereaktuje s materiálem objektu. Světlo se skládá z fotonů, které se přímočaře šíří od svého emitoru všemi směry. Při dopadu fotonu na objektu může na základě jeho materiálu dojít k třem možnosnostem, pohlcení fotonu, odražení nebo transmisi. Co lidské oko následně uvídí, závísí na světelných paprscích, které oko zásahne \cite{Light2009}.  
        
        Chování světla zachycují globální zobrazovací metody pro realistické vykreslení scén. Tyto metody jsou však výpočetně náročné, a proto je využívají hlavně off-line aplikace. Protože velká většina paprsků naše oko (kameru) vůbec nezasáhne, real-time aplikace světlo obvykle zjednodušují takovým způsobem, že světelné paprsky se vrhají z pozice oka a na základě toho, kam po odrzech dopadnou předají oku barvu objektu \cite{Light2009}. Přestože současné grafické karty začínají podporovat \emph{ray-tracing}, velká většina zařízení a softwaru stále využívá k zobrazování rasterizaci. 
    
        Kromě vrhání paprsků je další drahá operace výpočet průsečíků paprsků s objekty. Tato operace je navíc obvzlášt náročná pro zakomponování do rasterizačních \emph{pipeline}. Proto většina algoritmů pro osvětlování scén je založena na aproximaci průsečíků. Následující metody jsou proto zvolené, aby je bylo možné zakomponovat do rastareziční \emph{pipeliny}. 
                
        \subsection{Odrazy světla}
        
            Voda má reflektivní vlastnosti, a proto barva její hladiny zavísí na tom, odkud světelný paprsek, který následně zasáhne příjímač, přijde. Paprsek světla, resp. paprsek vrženého z příjímače (oko, kamera), se od ideálně reflektivního povrchu odráží pod stejným úhlem jako, pod kterým parsek dopadá \cite{LightDir2009}.
            
            Odrazy scény na vodní hladině lze levně simulovat pomocí tzv. \emph{cubemap}. Jedná se o druh textur, které májí tvar krychle. Hlavní výhodou užití \emph{cubemap} je její způsob vzorkování, které místo 2D souřadnice využivá 3D vektor. Jestliže se střed \emph{cubemapy} (krychle) napozicuje na počátek souřadnic, vzorkovací 3D vektor následně směřuje na jednu jeho stranu krychle a podle toho, kde vektor protne stranu, převezme fragment barvu textury. Pro dosažení efektu odrazu scény pak stačí jako vzorkovací vektor zvolit vektor odrazu \cite{ReflectCubemap2022}. 
        ...
        \subsection{Útlum světla}
            Potom, co světlo protne hladinu, jeho paprsek může prochází ve vodě dvěma transformacemi, může dojít k rozptylu parsku nebo k absorbci světla. Absorbci způsobuje nejen délka dráhy paprsku a látky obsažené ve vodě, ale také mikroorganismů žíjících, které v ní žijí\cite{Akkaynak2017}. Jev, který je predominantním důsledkem absorbce, je např. distorze barevného spektra \cite{Fleck2007}. Na základě obsah vody jsou různé vlnové délky pohlceny. V moři kvůli vysokém obsahu soli dochází k vysoké absorbci barev s dlouhými vlnovými délkami jako červená, zatímco u vody s vysokým obsahem chlorofylu pohlcují vše kromě zelených barev \cite{Cerezo2002}. Protože mnoho faktorů hraje roli v absorbci, musí chování značně zjednodušit, aby je počítače stihly vypočítat. Baboud a Décoret \cite{Baboud2006} modelují záření s vlnovou délkou $\lambda$ odraženého z bodu ve vodě $p_g$ do bodu na hladině $p_s$ jako
            ...
            \noindent kde $\omega$ značí směr od $p_w$ do $p_s$, $L_\lambda(p_w, \omega)$ je odchozí záření z bodu $p_w$ ve směru $\omega$, $d$ je vzdálenost $p_w$ a $p_s$, konstanta $L_{d \lambda}$ reprezentuje záření z rozptylu světla, $\alpha_\lambda$ je koeficient exponenciální útlum závislý na $d$
            
            \begin{equation}
                \alpha_{\lambda}(d) = e^{-a_{\lambda} d},
            \end{equation}

            \noindent kde $a_\lambda$ útlumová konstanta závislá na vlasnostech vody. Protože obraz počítače pracuje v RGB barevné spektru rovnice \ref{eq:att} se musí pro jednotlivé kanály RGB
            \begin{equation}
        ...
        \subsection{Kaustiky}
            ...
            Kaustika je optický jev, který vzniká díky reflektivním a refraktivním vlastnostem vody. Kaustiky jsou komplexní vzory světla způsobené konvergencí odražených nebo zlomených paprsků světla do jednoho bodu \cite{Shah2007}. Jedná se oběžný optický jev, který vidět např. při osvětlení průhledné sklenice nebo pod hladinou moře (viz obrázek \ref{fig:caustics}).
            ...
            Zobravání kaustik je bohužel náročná operace, neboť pro její simulaci je třeba vypočítat průsečíky paprsků s objekty ve scénách. V případě, že objekty scény jsou jednoduché útvary jako kvádr nebo koule, lze takové průsečíky vypočítat \cite{Wallace2022}, ale tyto tvary se jen ojediněle vyskytují v hrách. Shah simuluje kaustiky skrz pomocnou mapu kaustik \cite{Shah2007}. Všechny výpočty pro aproximaci průsečíku počítá v prostoru obrazu. Výhodou této metody je, že dokáže reagovat na různě složité objekty a také na dynamické prostředí. Shahův algoritmus lze rozdělit na tyto části \cite{Shah2007}
            
            \begin{enumerate}
             \item \textbf{Získání souřadnic příjímacích objektů:} Shah nejdříve vykreslí příjímací objekty, tj. objekty, na kterých se kaustika může zobrazit, z pohledu světelného zdroje do textury, které místo barvy ukládá světové souřadnice. 
            ...
            Hlavní krok algoritmu je třetí bod a proto je v této části rozveden do většího detailu. Nejpodstatnější část tvorby mapy kaustik se nachází ve \emph{vertex shaderu}. Nejdříve Shah vykreslí body \cite{Shah2007}, které spolu vytváří mřížku o velikosti textury z druhého bodu, tj. o velikosti textury souřadnic a normál refraktovaného bodu. Učelem tété velikost je, aby pro každý takovýto bod náležel jeden texel textury refraktovaného objektu. Tyto body následně promítne na příjímací objekty. Ve \emph{vertex shaderu} by tento postup vypadal takto
            
            \begin{verbatim}
Pro každý vrchol V proveď:
    r = lomený směr světla
    P = OdhadPrůsečíku(pozice V, směr r, textura příjímacích objektů)
    vrať P
            \end{verbatim}
            ...
            \noindent kde $V$ je světová pozice vrcholu \verb|V|, $P$ je pozice \verb|P|, $\mathbf{r}$ je směr \verb|r| a $d$ je délka vektoru $\mathbf{r}$. Úloha odhadu průsečíku je pak podle rovnice \ref{eq:approx} přeformulována na odhad délky $d$, protože kdyby byla hodnota $d$ rovna vzdálenosti $P$ od $V$, pak by rovnice \ref{eq:approx} popisovala reálný průsečík \verb|P|. 
                
            Shahův konečný odhad průsečíku $P_{2}$ vypočítává dvěma iteracemi vztahu \ref{eq:approx}. Nejdříve za $d$ dosadí hodnotu jedna. Poté vypočítá pozici $P_1 = V + 1 \cdot \mathbf{r}$, jehož souřadnice převede do prostoru světla a následně promítne na texturu příjímacích objektů. Promítnutím vrcholu $P_1$ zjistí jeho světovou souřadnici. Poslední odhad $d$ poté zvolí jako vzdálenost vrcholu $P_1$ od $V$. V pseudokódu vypadá funkce \verb|OdhadPrůsečíku| takto \cite{Shah2007}
            
            \begin{verbatim}
OdhadPrůsečíku(float3 V, float3 r, texture2D recievers) 
    P_1 = V + 1 * r
    // převod souřadnic P_1 do systému souřadnic kamery 
    cameraP_1 = maticeViewProj * float4(P_1, 1)  
    // promítnutí bodu P_1 do příjímacích objektů
    uvP_1 = 0.5 * (P_1.xy / P_1.w) + (0.5, 0.5)
    // zjistění světových souřadnic P_1
    worldP_1 = texture(recievers, uvP_1)
    
    P_2 = V + distance(V, worldP_1.xyz)
    cameraP_2 = maticeViewProj * float4(P_2, 1)
    uvP_2 = 0.5 * (P_2.xy / P_2.w) + (0.5, 0.5)
    worldP_2 = texture(recievers, uvP_1)
    vrať worldP_2
            \end{verbatim}

\section{Analýza technologií}
