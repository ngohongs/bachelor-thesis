   ...
   Pro každé i, j proveď
    ...
    \section{Optické vlastnosti}
        ...
        \subsection{Fresnelovy rovnice}
             ...
             \begin{equation}
              \begin{split}
                ...
                R_{\perp} &= \left( \frac{n_1\cos(\theta_2) - n_2\cos\theta_1}{n_1\cos(\theta_2) + n_2\cos\theta_1} \right)^2 \\
                \\
            ...
            \noindent Koeficient $R$ popisuje intenzitu odraženého světlo, kdežto koeficient $T$ říká intenzitu refraktovaného světla \cite{ReRe2009}. Úhly $\theta_1$ a $\theta_2$ jsou úhly dopadu příchozího paprsku a lomeného paprsku. Konstanty $n_1$ a $n_2$ jsou IOR příchozího a odchozího prostředí. V realitě lze tento optický fenomém nejvíce vidět u mělkých stojatých vod (viz obrázek \ref{fig:fresnel}). Místa, na která se díváme pod malým úhlem ($\theta_1$ v obrázku \ref{fig:reflect}), vídíme spíše průhledně, zatímco v místech, na která se díváme pod velkým úhlem, vidíme spíše odraz okolní prostředí.
        ...
        \subsection{Útlum světla}
            Potom, co světlo protne hladinu, jeho paprsek prochází ve vodě k dvěma transformacím. Nejprve se světlo ve vodě rozptyje a dále dochází k absorbci světla. Absorbci fotonů a to nejen kvůli jeho délce, ale také kvůli mikroorganismů žíjících ve vodě \cite{Akkaynak2017}. Barva výsledného obraz, tj. konečné světelné paprsky, které zasáhnou naše oko, je
        \subsection{Kaustiky}
