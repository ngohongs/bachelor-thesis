            ...
            \subsection{Částicové metody}  
                ...
                \subsection{Hybridní metody}
                    ...
                    \subsubsection{Simulace podle vlnové rovnice}
                        Vlnová rovnice nahlíží na vodní hladinu jako na elastickou membránu, která nebere v potaz, co se pod ní děje. Protože problematiku redukuje do dvourozměrného prostoru, místo NSE využívá pro fyzikální korektní pohyb pouhý Newtonův druhý zákon. 
                        
                        Síla, která vyvolává pohyb, je v následující části vyvozená. Pro jednoduchost je problém redukován na 1D, kde vypadá vlna jako vibrující napnutá struna (viz. obrázek \ref{fig:wave_eq}), postup lze analogicky převést do 2D. Jednodimenzionální vlnová rovnice předpokládá, že jsou splněny tyto podmínky \cite{WaveEqDerivation}:
                        \begin{enumerate} \label{en:wave_as}
                         ...
                         \item výchylky struny v horizontální ose, ose x, jsou zanedbatelné (struna se hýbe jen v vertikální ose y),
                         \item vertikální výchylky jsou malé a sklony struny od osy x jsou velmi malé.
                        \end{enumerate}
                        ...
                        Nechť funkce $u(x,t)$ udává, jak je struna na pozici $x$ v čase $t$ vychýlená od osy x . V případě, že je struna v klidové stavu, struna splývá s osou x. Dále nechť je segment struny $AB$ velmi malý, na který z obou stran působí síly $F_T$ (napínací síla).   
